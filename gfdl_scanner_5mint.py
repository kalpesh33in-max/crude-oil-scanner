...
def get_bias_label(net):
    ...
    if net>150: return "🚀 STRONG BULLISH"
    if net>0: return "🟢 Mild Bullish"
    if net<-500: return "🔥 VERY STRONG BEARISH"
    if net<-150: return "📉 STRONG BEARISH"
    if net<0: return "🔴 Mild Bearish"
    return "⚖ Neutral"
...
def parse_alert(text):
    ...
    future=float(future_match.group(1)) if future_match else None
...
async def process_summary(context:ContextTypes.DEFAULT_TYPE):
    ...
    if not alerts_buffer or window_start is None:
        return
