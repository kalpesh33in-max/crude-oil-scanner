...
# Symbol Range = Strike Price Difference
ATM_RANGE = {
    "BANKNIFTY": 100,
    "HDFCBANK": 5,
    "ICICIBANK": 10,
...
def classify_strike(symbol, strike, option_type, future_price):
    ...
    # ATM: Strike is within one 'strike difference' of the future price
    if abs(strike - future_price) <= width:
        return "ATM"

    # CALL (CE) Classification
    if option_type == "CE":
        # ITM: Strike is lower than current price (already in profit)
        return "ITM" if strike < (future_price - width) else "OTM"

    # PUT (PE) Classification
    if option_type == "PE":
        # ITM: Strike is higher than current price (already in profit)
        return "ITM" if strike > (future_price + width) else "OTM"
...
def parse_alert(text):
    # Regex to capture based on your specific alert structure
...
async def process_summary(context: ContextTypes.DEFAULT_TYPE):
    ...
    for symbol in TRACK_SYMBOLS:
        ...
        # FUTURE PRICE ON TOP AS REQUESTED
        ...
        fb, fs = data[symbol]["FUTURE_BUY"]["TOTAL"], data[symbol]["FUTURE_SELL"]["TOTAL"]
        message += "-" * 55 + "\n"
