...
def parse_alert(text):
    ...
    if "WRITER" in text_upper:
        ...
        action_type = "FUTURE_SC" if "FUTURE" in text_upper else ("CALL_SC" if option_type == "CE" else "PUT_SC")

    elif "LONG UNWINDING" in text_upper:
        action_type = "FUTURE_UNW" if "FUTURE" in text_upper else ("CALL_UNW" if option_type == "CE" else "PUT_UNW")
    ...
    for symbol in TRACK_SYMBOLS:
        ...
        if symbol in opt_data:
            ...
            for act in opt_data[symbol]:
                ...
                display = act.replace("CALL_WRITER","CALL_WR").replace("PUT_WRITER","PUT_WR")

                message += f"{display[:8]:8}{f'{itm_l}({format_money(itm_t)})':>14}{f'{otm_l}({format_money(otm_t)})':>14}{f'{tot_l}({format_money(tot_t)})':>14}\n"
        ...
        if symbol in fut_data:
            ...
            for act in fut_data[symbol]:
                ...
                display = format_future_name(act)

                message += f"{display:10} : {lots} lots ({format_money(turn)})\n"
