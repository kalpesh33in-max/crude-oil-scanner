...
BOT_TOKEN_2 = os.getenv("SUMMARIZER_BOT_TOKEN_2") # Second Bot Token
TARGET_CHANNEL_ID = os.getenv("TARGET_CHANNEL_ID")
# Target chat IDs for the two separate reports
SUMMARY_2MIN_CHAT_ID = os.getenv("SUMMARY_2MIN_CHAT_ID")
SUMMARY_5MIN_CHAT_ID = os.getenv("SUMMARY_5MIN_CHAT_ID")

# Initialize second bot instance (will be used in 5-min summary)
bot2 = Bot(token=BOT_TOKEN_2) if BOT_TOKEN_2 else None

# Dual buffers for separate reporting intervals
buffer_2min = []
buffer_5min = []
...
def parse_alert(text):
    ...
    if "WRITER" in text_upper:
        ...
        if symbol_full.endswith("-I") or "FUTURE" in text_upper: action_type = "FUTURE_SC"
        else: action_type = "CALL_SC" if option_type == "CE" else "PUT_SC"
    elif "LONG UNWINDING" in text_upper:
        if symbol_full.endswith("-I") or "FUTURE" in text_upper: action_type = "FUTURE_UNW"
...
async def process_2min_summary(context: ContextTypes.DEFAULT_TYPE):
    ...
    # Safety: Swap buffer to avoid missing messages
    batch, buffer_2min = buffer_2min, []
...
async def process_5min_summary(context: ContextTypes.DEFAULT_TYPE):
    ...
    # Use Bot 2 if configured, otherwise fallback to Bot 1
